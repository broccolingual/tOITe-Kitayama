...
class ClsWindow:
            ...
            for strWindowName in self.tplWindowName:
                ...
                if strWindowName == 'full':
                    cv2.setWindowProperty(
                        strWindowName, cv2.WND_PROP_FULLSCREEN, cv2.WINDOW_FULLSCREEN)
            self.active = True
    ...
    def prepareFullScreen(self, sMonitorWidth, sMonitorHeight, sImageWidth, sImageHeight):
        ...
        if sRatioWidth > sRatioHeight:
            ...
            self.sBlankMode = 1
            sBlankWidth = (int)(
                (sMonitorWidth - self.sRatioFull * sImageWidth) / 2)
            ...
            self.sBlankMode = 2
            sBlankHeight = (int)(
                (sMonitorHeight - self.sRatioFull * sImageHeight) / 2)
            self.imBlank = np.uint8(np.zeros((sBlankHeight, sMonitorWidth, 3)))
            self.sHeightDiff = sMonitorHeight - \
                self.sRatioFull * sImageHeight - 2 * sBlankHeight
        else:
            self.sRatioFull = sRatioWidth
            self.sBlankMode = 0
    ...
    def imshow(self, strWindowName, imDisplay):
        if self.active == True:
            if strWindowName == 'full':
                ...
                sWidthFit = (int)(self.sRatioFull *
                                  imDisplay.shape[1] + self.sWidthDiff)
                sHeightFit = (int)(self.sRatioFull *
                                   imDisplay.shape[0] + self.sHeightDiff)
                imDisplay = cv2.resize(imDisplay, (sWidthFit, sHeightFit))
                if self.sBlankMode == 1:
                    imDisplay = np.concatenate(
                        [self.imBlank, imDisplay, self.imBlank], 1)
                elif self.sBlankMode == 2:
                    imDisplay = np.concatenate(
                        [self.imBlank, imDisplay, self.imBlank], 0)
